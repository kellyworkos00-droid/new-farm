...
export async function POST(request: NextRequest) {
  try {
    ...
    const { coopId, date, quantity, grade, notes } = await request.json();

    if (!coopId || !date || quantity === undefined) {
      return NextResponse.json(
        { error: 'Coop ID, date, and quantity are required' },
        { status: 400 }
      );
    }
    ...
    const eggRecord = await prisma.eggProduction.create({
      data: {
        farmId: farm.id,
        coopId,
        date: new Date(date),
    ...
    return NextResponse.json(eggRecord, { status: 201 });
  } catch (error) {
